...
def KeyPoint_LessThan(kpt1,kpt2):
...
class SIFT:
    ...
    def createInitialImage(self,image_in):
        '''
    ...
    def buildGaussianPyramid(self,image_in,nOctaves):
        '''
        ...
        for o in range(nOctaves):
            one_octave = np.zeros((image.shape[0]//(2**o),image.shape[1]//(2**o),(self._nOctaveLayers + 3)),dtype = np.float32)
            for l in range(self._nOctaveLayers + 3):
                if o==0 and l==0:
                    ...
                    one_octave[:,:,l] = cv2.resize(src,(one_octave.shape[1],one_octave.shape[0]),interpolation = cv2.INTER_AREA)

                else:
                    src_layer = l-1
                    one_octave[:,:,l] = cv2.GaussianBlur(one_octave[:,:,src_layer],(0,0),sig[l]) #sig[l]相当于sig_diff
            pyr[o] = one_octave
        return pyr
    
    def buildDoGPyramid(self, gpyr):
        '''
        每一组图片之间求差
        '''
        dogpyr = {}
    ...
    def adjustLocalExtrema(self,dogpyr,kpt,octave,layers,l,r,c):
        ...
        for i in range(self.SIFT_MAX_INTERP_STEPS):
            ...
            dxy = (layers[r+1,c+1,l] - layers[r+1,c-1,l] - layers[r-1,c+1,l] + layers[r-1,c-1,l]) * cross_deriv_scale
            dxs = (layers[r,c+1,l+1] - layers[r,c-1,l+1] - layers[r,c+1,l-1] + layers[r,c-1,l-1]) * cross_deriv_scale
            dys = (layers[r+1,c,l+1] - layers[r-1,c,l+1] - layers[r+1,c,l-1] + layers[r-1,c,l-1]) * cross_deriv_scale
    ...
    def calcOrientationHist(self,img_gaussion,r1,c1,radius,sigma,hist,n):
    ...
    def findScaleSpaceExtrema(self,gpyr,dogpyr):
                    ...
                    for c in range(self.SIFT_IMG_BORDER, cols -self.SIFT_IMG_BORDER):
                        ...
                        if abs(cur_val) > threshold and\
                            ...
                            or(cur_val<threshold and cur_val == np.min(neighbor) )):
                                kpt = KeyPoint()
                                r1,c1,isKpt = self.adjustLocalExtrema(dogpyr,kpt,octave,layers,i,r,c,)
                                if isKpt == False:
                                    continue
                                ...
                                omax = self.calcOrientationHist(img_gaussion,r1,c1,radius,sigma,hist,n)
                                #print(omax,hist)
    ...
    def removeDuplicated(self,keypoints):
        keypoints = sorted(keypoints, key=functools.cmp_to_key(KeyPoint_LessThan))
        n = len(keypoints)
        kpidx = np.arange(n)
    ...
    def retainBestkeypoints(self,keypoints):
    ...
    def calcSIFTDescriptor(self,img,x,y,ori,scl,d, n,scale):
        x = int(round(x))
        y = int(round(y))
        # print(ori,"rotate---------------------------->")
        ...
        for i in range(-radius,radius+1):#y,row
            for j in range(-radius,radius+1):#x,col
                ...
                if rbin > -1 and rbin < d-0.1 and cbin > -1 and cbin < d-0.1 and \
    ...
    def calcDescriptors(self, gpyr, keypoints):
        ...
        for i in range(len(keypoints)):
            ...
            hist,box = self.calcSIFTDescriptor(img,x,y,angle,size*0.5,d, n,scale)
            boxes.append(box)
            des = {'x':kpt.x,
                   'y':kpt.y,
                   'hist':hist}
    ...
    def run_sift_feaures(self, image):
        ...
        initial_image = self.createInitialImage(image) 
        ...
        gpyr = self.buildGaussianPyramid(initial_image,nOctaves)
        dogpyr = self.buildDoGPyramid(gpyr)
        # for key, value in dogpyr.items():
        #     for i in range(value.shape[2]):  
        #          plt.imshow(dogpyr[key][:,:,i],"gray"),plt.title("gpyr"),plt.show()
        keypoints = self.findScaleSpaceExtrema(gpyr,dogpyr)
        print("去重前",len(keypoints))
        keypoints = self.removeDuplicated(keypoints)
        print("去重后",len(keypoints))    
        keypoints = self.retainBestkeypoints(keypoints)
        ...
        for i in range(len(keypoints)):
            ...
            #print(temp.x,temp.y)
            keypoints[i] = temp
        descriptors,boxes = self.calcDescriptors(gpyr, keypoints)
